...
select o.dim_cuentas, o.dni, o.apertura, o.antiguedad, o.limite_credito, (p.renta)/4 rentabilidad, q.nivel_riesgo_trim, o.frecuencia, o.recencia, o.monto

from

(select f.dim_cuentas dim_cuentas, f.dni dni, g.apertura apertura, g.antiguedad antiguedad, f.tipo_cuenta tipo_cuenta, g.limite_credito limite_credito, f.frecuencia frecuencia, """
...
"""
left join 
(SELECT A.ATR_DNI DNI, STATS_MODE(B.NIVEL_RIESGO) NIVEL_RIESGO_TRIM
FROM DW.FAC_NIVEL_RIESGO A
LEFT JOIN
DW.DIM_NIVEL_RIESGO B
ON B.ID = A.ATR_NIVEL_RIESGO_ID """
f'WHERE a.DIM_TIEMPOS BETWEEN {ff_int} AND {fi_int} '
"""GROUP BY A.ATR_DNI ) q
on q.dni = o.dni

LEFT JOIN

(select atr_dni dni, dim_cuentas, met_rent_markov_ajuste_inf renta

from dw.fac_rentacum """

f'where dim_tiempos = {fi_int}) p on p.dim_cuentas = o.dim_cuentas ' 
"""
